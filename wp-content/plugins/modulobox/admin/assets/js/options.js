...
/* global jQuery, mobx_localize, mobx_google_fonts,	CodeMirror, ajaxurl, wpColorPickerL10n, Color */
(function( $ ) {
    ...
    $(function() {
		...
		var addStyles = function( css ) {
			...
			var $styles = $( 'style#mobulobox-inline-css' );

			if ( $styles.length < 1 ) {

				$styles = $('<style/>', {
					id  : 'mobulobox-inline-css',
					rel : 'stylesheet'
				}).appendTo('head');
		...
		var addFonts = function( url ) {

			var $styles = $( 'link#mobulobox-google-fonts' );

			if ( $styles.length < 1 ) {

				$styles = $('<link/>', {
					id  : 'mobulobox-google-fonts',
					rel : 'stylesheet'
				}).appendTo('head');
		...
		$( document ).on( 'click', '#modulobox input[name="save"], #modulobox input[name="preview"], .mobx-register-button, .mobx-check-update-button', function() {
			...
			if ( name === 'save' ) {
				...
				message = mobx_localize[ $(this).data( 'msg' ) ];
				data    = {
					nonce  : mobx_localize.nonce,
					action : 'modulobox_ajax_request',
					type   : $(this).data( 'type' ),
			...
			xhr = $.ajax({
				...
				success : function( data ) {
					...
					if ( $this.attr( 'name' ) === 'save' && data.success && data.content ) {
						...
						addStyles( data.content.inlineCSS );
						addFonts( data.content.googleFonts );
						accessibility = data.content.accessibility;

						// Disable history in preview mode
						data.content.options.history = false;
						...
						$( '.mobx-updates-support-content' ).html( $( data.content ).html() );
		...
		$( document ).on( 'click', '#modulobox .mobx-update-button', function() {

			$modalMessage.html( mobx_localize.updating_msg );
			$modalHolder.removeClass('mobx-success mobx-error' );
			$modalHolder.addClass( 'mobx-modal-show' );

		});

		var fontList     = document.createDocumentFragment(),
			fontFamily   = document.getElementsByClassName( 'mobx-font-families' ),
			fontVariants = document.getElementsByClassName( 'mobx-font-variants' ),
			fontSubsets  = document.getElementsByClassName( 'mobx-font-subsets' ),
			stdVariants  = [],
			option;

		// Save default variants
		if ( fontVariants[0] ) {
			stdVariants = Array.prototype.slice.call( fontVariants[0].childNodes );
		}

		// Prepare Google fonts
		if ( typeof mobx_google_fonts !== 'undefined' ) {

			for ( var font in mobx_google_fonts ) {

				if ( mobx_google_fonts.hasOwnProperty( font ) ) {
		...
		// Set Google font subsets and variants
		var setGoogleFont = function( e ) {

			var el    = e.target,
				font  = el.value,
				field = el.parentElement;

			setGoogleType( field, font, 'subsets' );
			setGoogleType( field, font, 'variants' );

		};

		// Fill select subsets and variants dropdown list
		var setGoogleType = function( el, font, type ) {

			var prop = document.createDocumentFragment(),
				select = el.getElementsByClassName( 'mobx-font-' + type )[0];
			...
			if ( mobx_google_fonts.hasOwnProperty( font ) ) {
				...
				if ( font.hasOwnProperty( type ) ) {
					...
					for ( var i = 0; i < type.length; i++ ) {
						...
						var text = type[i].split( /(\d+)/ ).filter( Boolean );
						option = document.createElement( 'option' );
						option.textContent = text[0] + ( text[1] ? ' ' + text[1] : '' );
						option.value = type[i];
				...
				for ( var y = 0; y < stdVariants.length; y++ ) {
					prop.appendChild( stdVariants[y].cloneNode( true ) );
				}
		...
		var event = 'createEvent' in document ? 'create' : 'fire';	

		// Add change event to Google Font family select
		for ( var i = 0; i < fontFamily.length; i++ ) {

			fontFamily[i].appendChild( fontList.cloneNode( true ) );
			fontFamily[i].addEventListener( 'change', setGoogleFont );

			var family = fontFamily[i].getAttribute( 'data-value' ),
				selected = fontFamily[i].querySelectorAll( '[value="' + family + '"]' );
			...
			if ( event === 'create' ) {
				...
				fontFamily[i].dispatchEvent( evt);
	
			} else {

				fontFamily[i].fireEvent( 'onchange' );

			}

			var variant = fontVariants[i].getAttribute( 'data-value' );
				variant = fontVariants[i].querySelectorAll( '[value="' + variant + '"]' );

			if ( variant[0] ) {
				variant[0].setAttribute( 'selected', 'selected' );
			}

			var subset = fontSubsets[i].getAttribute( 'data-value' );
				subset = fontSubsets[i].querySelectorAll( '[value="' + subset + '"]' );

			if ( subset[0] ) {
				subset[0].setAttribute( 'selected', 'selected' );
			}
