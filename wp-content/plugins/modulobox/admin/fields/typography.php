...
if ( ! class_exists( 'ModuloBox_Typography_Field' ) ) {
	...
	class ModuloBox_Typography_Field extends ModuloBox_Settings_field {
		...
		static function font_variants( $size, $args ) {
			...
			echo '<select class="mobx-select mobx-font-variants" title="' . esc_attr__( 'Font Weight' , 'modulobox' ) . '" name="' . esc_attr( $args['name'] )  . '[' . esc_attr( $size )  . '][font-weight]" data-value="' . esc_attr( $args['value'][ esc_attr( $size ) ]['font-weight'] ) . '">';
				...
				foreach( self::$std_weights as $weight ) {
					echo '<option value="' . esc_attr( $weight ) . '">' . esc_html( $weight ) . '</option>';
				}
		...
		static function scripts() {
			...
			wp_localize_script( MOBX_NAME . '-options', MOBX_SLUG . '_google_fonts', require_once( MOBX_INCLUDES_PATH . 'google-fonts.php' ) );
		...
		static function sanitize( $val, $args ) {
			...
			$val = (array) json_decode( json_encode( $val ), true );

			foreach( $val as $size => $prop ) {

				if ( array_key_exists( $size, $args['default'] ) ) {

					$size = sanitize_key( $size );

					// Make sure required fields are present (to generate inline css)
					$val[ $size ]['font-style']      = isset( $val[ $size ]['font-style'] ) ? $val[ $size ]['font-style'] : 'normal';
					$val[ $size ]['text-decoration'] = isset( $val[ $size ]['text-decoration'] ) ? $val[ $size ]['text-decoration'] : 'none';
					$val[ $size ]['text-transform']  = isset( $val[ $size ]['text-transform'] ) ? $val[ $size ]['text-transform'] : 'none';

					// Globally sanitize standard text/select/radio field
					$val[ $size ] = array_map( 'sanitize_text_field', (array) $val[ $size ] );

					// Sanitize line-height & font-size
					$font_args = array( 'min' => 1, 'max' => 60, 'step' => 1 );
					$val[ $size ]['line-height'] = isset( $val[ $size ]['line-height'] ) ? ModuloBox_Number_Field::sanitize( $val[ $size ]['line-height'], $font_args ) : $args['default'][ $size ]['line-height'];
					$val[ $size ]['font-size']   = isset( $val[ $size ]['font-size'] ) ? ModuloBox_Number_Field::sanitize( $val[ $size ]['font-size'], $font_args ) : $args['default'][ $size ]['line-height'];

					// Sanitize color
					$color_args = array( 'default' => $args['default'][ $size ]['color'], 'alpha' => false );
					$val[ $size ]['color'] = isset( $val[ $size ]['color'] ) ? ModuloBox_Color_Field::sanitize( $val[ $size ]['color'], $color_args ) : $args['default'][ $size ]['color'];	
