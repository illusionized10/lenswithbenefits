...
class ModuloBox_Init {
	 ...
	 * Because wp_localize_script converts numbers/booleans into strings (which is not compliant with JavaScript options format)
	 *
	 * @since 1.0.0
	 * @access public
	 */
	public function set_options() {
		...
		$svg_icons = apply_filters( MOBX_NAME . '_svg_icons_url', MOBX_PUBLIC_URL . 'assets/icons/modulobox.svg' );
		...
		$this->inline_script .= 'var mobx_svg_icons = \'' . esc_url( $svg_icons ) . '\';';
		$this->inline_script .= 'var mobx_google_fonts = \'' . $options['googleFonts'] . '\';';
	...
	public function add_custom_js_before() {
		...
		$customJS = $this->options['customJSBefore'];
		$this->inline_script .= $customJS ? "\n" . '// Custom JS Before module' . "\n" . $customJS . "\n" : '';
	...
	public function add_custom_js_after() {
		...
		$customJS = $this->options['customJSAfter'];
	...
	public function enqueue_scripts() {
		...
		// Enqueue Mediaelement player if enabled
		if ( $this->options['options']['mediaelement'] ) {
